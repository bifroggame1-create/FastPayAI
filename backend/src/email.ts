...
const SMTP_FROM = process.env.SMTP_FROM || 'FastPay <noreply@fastpay.ai>'
...
`
  ...
  return sendEmail({
    to: params.email,
    subject: `Заказ ${params.orderNumber} подтвержден - FastPay`,
    html,
...
`
  ...
  return sendEmail({
    to: params.email,
    subject: `Оплата заказа ${params.orderNumber} получена - FastPay`,
    html,
...
`
  ...
  return sendEmail({
    to: adminEmail,
    subject: `Новый заказ ${params.orderNumber} - FastPay`,
    html,
